...
def extract(path_img, path_save):
    global detector
    if detector is None:
    ...
    dir_skeleton = os.path.dirname(path_save)
    if not os.path.exists(dir_skeleton):
        os.makedirs(dir_skeleton, exist_ok=True)
    img_tgt.save(path_save)

def extract_dwpose(batch_data):
    for imgpath, skeletonpath in zip(batch_data["image_list"], batch_data["tgt_list"]):
        if not os.path.exists(imgpath) or os.path.exists(skeletonpath):
            continue
        global detector
        if detector is None:
            detector = DWposeDetector()
        ...
        result, score = detector(img_pil, detect_resolution=128)
        img_tgt = result.resize(img_pil.size)
        dir_skeleton = os.path.dirname(skeletonpath)
        if not os.path.exists(dir_skeleton):
            os.makedirs(dir_skeleton, exist_ok=True)
        img_tgt.save(skeletonpath)
